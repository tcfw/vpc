...
func (s *Listener) AddVLAN(vnid uint32, vlan uint16) error {
	...
	return netlink.BridgeVlanAdd(s.vteps[vnid].iface, vlan, false, false, false, false)
}
...
func (s *Listener) DelVLAN(vnid uint32, vlan uint16) error {
	...
	if s.vlans[vlan] <= 0 {
		return netlink.BridgeVlanDel(s.vteps[vnid].iface, vlan, false, false, false, false)
	}
